...
用户导入的图片会复制到 `data\icons\...`，因此删除原始导入图片后，musuka 仍会使用内部备份图片。
...
- 拟桌面显示替换图片，不显示文件名称。
- 单击选中、拖动保存位置、双击打开。
- 右键菜单：打开、打开所在位置、返回 settings、退出 musuka。
...
- 替换图片在 desktop 阶段会按 160 像素最大边长等比绘制；推荐使用带透明 alpha 通道的 PNG。
